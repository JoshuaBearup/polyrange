// Metafile leakage — per-tier defence contract.
// A signature WAF does not defend against a clean GET to /robots.txt or to a
// listed path (no malicious payload). The naive recon succeeds at T0 and T1.
// The real mitigation is "don't list sensitive paths in metafiles" — a
// configuration finding, not a payload filter.
