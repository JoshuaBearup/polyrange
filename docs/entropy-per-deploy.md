...
(SQLi)**, with 5 positions in its `INJECTION_LOCATIONS` pool (query, body-form,
body-json, header, cookie — path-segment deferred to v1.1 because the runtime's
`matchesRequest` needs template-matching support across all classes). Walking
through the deterministic seed-space per deploy:
...
══════════════════════════════════════════════════════════════════════════
  ...
  SQLi class-specific axes:
    ...
    injection-loc     ×   5        (query, body-form, body-json, header, cookie)
                      ─────
    SQLi axes         = 15
...
For SQLi specifically:
    ...
    L(SQLi) = 5
    A(SQLi) = 3   (dialect)
    Entropy(deploy, SQLi) ≈ 2.28 × 10¹⁴ × LLM_variance
...
- An attacker probing only `?q=` catches at most 1/5 of seeded deploys
  (the query-location ones) — even if dialect and theme don't matter to them,
  they still miss 80% on the position axis alone.
